...
/**
 * AC-014 — permission gating of the Business Functions page. The generic
...
describe('BusinessFunctionsPage — permission gating (AC-014)', () => {
  it('shows the forbidden fallback and does not mount the table without viewAny', () => {
    ...
    expect(
      screen.getByText("You don't have permission to view business functions."),
    ).toBeInTheDocument()
    expect(
      screen.queryByRole('region', { name: 'table-business-functions' }),
    ).not.toBeInTheDocument()
  })

  it('mounts <TableView domain="business-functions"> with viewAny', () => {
    canMock.mockImplementation((permission) => permission === 'business-functions.viewAny')

    renderPage()

    expect(
      screen.getByRole('region', { name: 'table-business-functions' }),
    ).toBeInTheDocument()
    expect(
      screen.queryByText("You don't have permission to view business functions."),
    ).not.toBeInTheDocument()
  })
})
