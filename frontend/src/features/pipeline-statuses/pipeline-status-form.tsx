...
import { usePipelineStatusFormMeta } from '@/features/pipeline-statuses/use-pipeline-status-form-meta'
import { PipelineStatusFormBody } from '@/features/pipeline-statuses/pipeline-status-form-body'
 ...
 * instance detail, create mode from `GET /meta/pipeline-statuses` — then
 * hands off to `PipelineStatusFormBody`, which reads every field from that
 * context via `MetaField`/`useResourcePermissions()`.
 */
export function PipelineStatusForm(props: PipelineStatusFormProps) {
  const { t } = useTranslation()
  const meta = usePipelineStatusFormMeta(props.mode)
