...
import { useProjectStatusForm } from '@/features/project-statuses/use-project-status-form'
import { CustomFieldsSection } from '@/features/custom-fields/CustomFieldsSection'
import type { ProjectStatusDetail, ProjectStatusFormMode } from '@/features/project-statuses/types'

interface ProjectStatusFormBodyProps {
 ...
 * non-render logic lives in `useProjectStatusForm`. `<CustomFieldsSection>`
 * (spec 0021) mounts the resource's admin-defined custom fields with zero
 * project-statuses-specific rendering/validation logic.
 */
export function ProjectStatusFormBody({ mode, onSuccess, onCancel }: ProjectStatusFormBodyProps) {
  const { t } = useTranslation()
  const { field: fieldPermission } = useResourcePermissions()
  const { form, serverError, onSubmit } = useProjectStatusForm({ mode, onSuccess })
      ...
      <Form {...form}>
        ...
        >
          {identityVisible && (
            <FormSection
              icon={Flag}
              title={t('projectStatuses.form.sections.identity.title')}
              description={t('projectStatuses.form.sections.identity.description')}
            >
              <MetaField
                control={form.control}
                name="name"
                metaKey="name"
                label={t('projectStatuses.form.name')}
              >
              ...
              <MetaField
                ...
                name="color"
                metaKey="color"
                label={t('projectStatuses.form.color')}
              >
                {({ field, disabled }) => (
                  <FormControl>
                    <ColorTokenPicker
              ...
              <MetaField
                ...
                name="sort_order"
                metaKey="sort_order"
                label={t('projectStatuses.form.sortOrder')}
              >
                {({ field, disabled, readOnly }) => (
                  <FormControl>
                    <Input
          ...
          <CustomFieldsSection resource="project-statuses" control={form.control} />
          ...
          <div className="mt-auto flex justify-end gap-2 pt-2">
            ...
            >
              {t('projectStatuses.form.cancel')}
            </Button>
            <Button type="submit" disabled={form.formState.isSubmitting}>
              {form.formState.isSubmitting
                ? t('projectStatuses.form.saving')
                : t('projectStatuses.form.save')}
            </Button>
          </div>
        </form>
      </Form>
