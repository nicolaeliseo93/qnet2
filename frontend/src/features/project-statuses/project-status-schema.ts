...
function baseFields(t: TFunction) {
  return {
    name: z
      .string()
      .min(1, t('projectStatuses.form.nameRequired'))
      .max(NAME_MAX_LENGTH, t('projectStatuses.form.nameMax')),
    color: z.string().max(COLOR_MAX_LENGTH, t('projectStatuses.form.colorMax')),
    sort_order: z.coerce
      .number()
      .int(t('projectStatuses.form.sortOrderInvalid'))
      .min(0, t('projectStatuses.form.sortOrderMin')),
  }
}

/** Create schema. `customFieldsSchema` is the toolbox-built schema for `custom_fields` (spec 0021 AC-023). */
export function buildCreateProjectStatusSchema(
  t: TFunction,
...
export function buildUpdateProjectStatusSchema(
  t: TFunction,
  customFieldsSchema: CustomFieldsSchema,
) {
  return z.object({ ...baseFields(t), custom_fields: asCustomFieldsField(customFieldsSchema) })
...
export type CreateProjectStatusFormValues = z.infer<
  ReturnType<typeof buildCreateProjectStatusSchema>
>
export type UpdateProjectStatusFormValues = z.infer<
  ReturnType<typeof buildUpdateProjectStatusSchema>
>
