...
import ProjectStatusesPage from '@/pages/project-statuses-page'
 ...
 * the table, mounting `<TableView domain="project-statuses">`, and the
...
const deleteProjectStatusMock = vi.fn()
vi.mock('@/features/project-statuses/api', () => ({
  deleteProjectStatus: (...args: unknown[]) => deleteProjectStatusMock(...args),
...
beforeEach(() => {
  ...
  deleteProjectStatusMock.mockReset()
  toastSuccessMock.mockReset()
  toastErrorMock.mockReset()
})

describe('ProjectStatusesPage — permission gating (AC-045)', () => {
  it('shows the forbidden fallback and does not mount the table without viewAny', () => {
    ...
    expect(screen.queryByRole('region', { name: 'table-project-statuses' })).not.toBeInTheDocument()
  })

  it('mounts <TableView domain="project-statuses"> with viewAny', () => {
    canMock.mockImplementation((permission) => permission === 'project-statuses.viewAny')

    renderPage()

    expect(screen.getByRole('region', { name: 'table-project-statuses' })).toBeInTheDocument()
    expect(
...
describe('ProjectStatusesTable — delete (BR-4)', () => {
  it('shows the backend message on a 409 (status still in use)', async () => {
    deleteProjectStatusMock.mockRejectedValue(
    ...
    await waitFor(() => expect(deleteProjectStatusMock).toHaveBeenCalledWith(1))
    await waitFor(() =>
  ...
  it('shows the success toast and refreshes on a successful delete', async () => {
    deleteProjectStatusMock.mockResolvedValue(undefined)
