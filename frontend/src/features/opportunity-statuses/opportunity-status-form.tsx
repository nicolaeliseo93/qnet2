...
import { useOpportunityStatusFormMeta } from '@/features/opportunity-statuses/use-opportunity-status-form-meta'
import { OpportunityStatusFormBody } from '@/features/opportunity-statuses/opportunity-status-form-body'
import type {
 ...
 * instance detail, create mode from `GET /meta/opportunity-statuses` — then
 * hands off to `OpportunityStatusFormBody`, which reads every field from
 * that context via `MetaField`/`useResourcePermissions()`.
 */
export function OpportunityStatusForm(props: OpportunityStatusFormProps) {
  const { t } = useTranslation()
  const meta = useOpportunityStatusFormMeta(props.mode)
  ...
  return (
    <ResourcePermissionsProvider permissions={meta.permissions}>
      <OpportunityStatusFormBody {...props} />
