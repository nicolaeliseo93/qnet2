...
it('200 with the catalogue for users and roles, keys matching each resolver\'s fields()', function () {
    ...
    // `lead-statuses`, spec 0034 `import-runs`, spec 0039 `status-groups`,
    // all in the same generic registry (config/authorization.php), so this
    ...
    expect($resources->keys()->all())->toEqualCanonicalizing([
        ...
        'registries', 'pipeline-statuses', 'projects', 'campaigns', 'leads', 'lead-statuses', 'status-groups', 'import-runs',
    ]);
