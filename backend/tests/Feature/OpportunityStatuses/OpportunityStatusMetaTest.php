...
it('403 without opportunity-statuses.viewAny', function () {
    $actor = opportunityStatusUserWith([]);
    Sanctum::actingAs($actor);

    $this->getJson('/api/meta/opportunity-statuses')->assertForbidden();
});

it('200: field catalogue matches the frozen contract, in order (AC-010)', function () {
    $actor = opportunityStatusUserWith(['viewAny', 'create']);
    Sanctum::actingAs($actor);

    $response = $this->getJson('/api/meta/opportunity-statuses')
        ->assertOk()
...
it('200: create-context permissions.fields are editable when the actor may create (AC-010)', function () {
    ...
    $this->getJson('/api/meta/opportunity-statuses')
        ->assertOk()
...
it('permissions.actions maps delete/export/import to the resource permissions', function () {
    $actor = opportunityStatusUserWith(['viewAny', 'export']);
    Sanctum::actingAs($actor);
