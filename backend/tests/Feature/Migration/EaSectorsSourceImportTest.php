...
// ---------------------------------------------------------------------------
// EaSectorsSource — self-referential tree: create + old_id + parent remap
// ---------------------------------------------------------------------------

it('creates root and child sectors, remapping parent_id via old_id', function () {
    seedMigrationsConfig();
    Http::fake([
        fakeMigrationsBaseUrl().'/ea-sectors*' => Http::response([
            'items' => [
    ...
    $run = MigrationRun::factory()->create(['user_id' => $actor->id, 'source' => 'ea-sectors']);

    runMigrationJobFor($run);

    $root = EaSector::query()->where('old_id', 1)->first();
    $child = EaSector::query()->where('old_id', 2)->first();
...
it('re-importing the same sectors is idempotent (skip, no duplicate)', function () {
    ...
    $actor = migrationsSuperAdminActor();
    runMigrationJobFor(MigrationRun::factory()->create(['user_id' => $actor->id, 'source' => 'ea-sectors']));

    $secondRun = MigrationRun::factory()->create(['user_id' => $actor->id, 'source' => 'ea-sectors']);
    runMigrationJobFor($secondRun);

    expect(EaSector::query()->where('name', 'Services')->count())->toBe(1)
...
it('isolates a failed sector row (missing name) without blocking the valid one', function () {
    ...
    expect(EaSector::query()->where('name', 'Valid Sector')->exists())->toBeTrue();
