...
it('403 without referent-types.viewAny', function () {
    $actor = referentTypeUserWith([]);
    Sanctum::actingAs($actor);

    $this->getJson('/api/meta/referent-types')->assertForbidden();
});

it('200: field catalogue matches the frozen contract, in order', function () {
    $actor = referentTypeUserWith(['viewAny', 'create']);
    Sanctum::actingAs($actor);

    $response = $this->getJson('/api/meta/referent-types')
        ->assertOk()
...
it('200: create-context permissions.fields are editable when the actor may create', function () {
    ...
    $this->getJson('/api/meta/referent-types')
        ->assertOk()
...
it('permissions.actions maps delete/export/import to the resource permissions', function () {
    $actor = referentTypeUserWith(['viewAny', 'export']);
    Sanctum::actingAs($actor);
