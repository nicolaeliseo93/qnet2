...
/**
 * Validates the query for GET /api/vat-rates/for-select (ADR 0011), mirroring
 * SourceForSelectRequest.
 *
 * Authorization is intentionally NOT handled here (it stays in the controller
 * via authorize('viewAny', VatRate::class)). Pagination bounds mirror
 * BaseApiController::validateRequest (offset >= 0, 1 <= limit <= MAX_LIMIT).
 */
class VatRateForSelectRequest extends FormRequest
{
    public function authorize(): bool
    {
        // Authorization handled in the controller via the VatRatePolicy.
        return true;
    }
