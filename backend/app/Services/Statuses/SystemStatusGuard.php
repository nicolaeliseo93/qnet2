...
class SystemStatusGuard
{
     ...
     */
    public function assertDeletable(PipelineStatus|LeadStatus $status): void
    {
        if (! $status->isSystem()) {
            return;
        }
     ...
     * @throws HttpException 422
     */
    public function assertUpdatable(PipelineStatus|LeadStatus $status, array $submittedAttributes): void
    {
        if (! $status->isSystem()) {
            return;
        }
     ...
     * fallback assigned when a Lead/Project/(standalone) Campaign is created
