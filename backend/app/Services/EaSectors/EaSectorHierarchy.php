...
namespace App\Services\EaSectors;

use App\Models\EaSector;
use Illuminate\Support\Collection;

/**
 * Read-side resolution of the EA sector tree (spec 0018): the ancestor
 ...
 */
final class EaSectorHierarchy
{
    /**
     * Defensive cap on the ancestor walk: the write-side anti-cycle guard
     * (EaSectorService) prevents a real cycle from ever being persisted, so
     ...
     */
    public function ancestors(EaSector $sector): Collection
    {
        $chain = [];
        ...
        while ($currentId !== null && $depth < self::MAX_DEPTH) {
            $parent = EaSector::find($currentId);
     ...
     */
    public function isAncestorOf(EaSector $sector, int $candidateAncestorId): bool
    {
    ...
    /**
     * @param  Collection<int|string, Collection<int, EaSector>>  $byParent
     * @return array<int, array<string, mixed>>
     */
    private function buildNodes(Collection $byParent, ?int $parentId): array
    {
        ...
        /** @var Collection<int, EaSector> $children */
