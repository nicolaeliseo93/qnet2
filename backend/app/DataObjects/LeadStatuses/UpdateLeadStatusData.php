 ...
 * LeadStatusService contract is explicit. `color`/`group` are legitimately
 * nullable/optional VALUES (color clears back to none; group is simply not
 * always resubmitted on a partial PATCH), so a plain null property cannot
 * distinguish "not submitted" from "submitted as null" —
 * `colorSubmitted`/`groupSubmitted` carry that distinction explicitly,
 * mirroring UpdatePipelineStatusData's same pairs. spec 0039, D-5:
 * `sort_order` is GONE from this DTO — server-managed, never accepted from
 * the client (see App\Services\Statuses\StatusOrderManager). `group`
 * (pivot, App\Enums\StatusGroup) — App\Services\Statuses\SystemStatusGuard
 * rejects it outright when the target row is a system status.
 */
